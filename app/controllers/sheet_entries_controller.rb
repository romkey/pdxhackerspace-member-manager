class SheetEntriesController < AuthenticatedController
  ...
  def sync_to_users
    ...
    SheetEntry.where(user_id: nil).find_each do |sheet_entry|
      ...
      if matches.count == 1
        ...
        # Create RFID record if not already present
        if sheet_entry.rfid.present?
          Rfid.find_or_create_by!(user: user, rfid: sheet_entry.rfid)
        end
        ...
        if sheet_entry.rfid.present?
          Rfid.create!(user: user, rfid: sheet_entry.rfid)
        end
  ...
  def sync_to_user
    ...
    if matches.count == 1
      ...
      redirect_to sheet_entry_path(@sheet_entry), alert: "Multiple users match this sheet entry. Cannot automatically sync."
      return
    end
      ...
      matching_rfids.each do |rfid_record|
        ...
        if name_mismatch || email_mismatch
          ...
          name_in_sheet = user.full_name.present? && 
                          SheetEntry.where("LOWER(name) = ?", user.full_name.downcase).exists?
          email_in_sheet = user.email.present? && 
                           SheetEntry.where("LOWER(email) = ?", user.email.downcase).exists?
