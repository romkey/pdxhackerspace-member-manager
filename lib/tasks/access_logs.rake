namespace :access_logs do
  ...
  task import: :environment do
    directory = ENV["ACCESS_LOGS_DIRECTORY"]
    ...
    Dir.glob(File.join(directory, "*.log")).sort.each do |file_path|
...
def parse_log_line(line, file_year, original_line = nil)
  ...
  def parse_timestamp(timestamp_str, file_year)
    ...
    if timestamp_str.match?(/\A\d{4}-\d{2}-\d{2}T/)
      return Time.zone.parse(timestamp_str)
    end
